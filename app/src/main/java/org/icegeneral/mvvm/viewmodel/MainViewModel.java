...
import org.icegeneral.mvvm.utils.DateUtils;
...
public class MainViewModel extends BaseViewModel {
    ...
    public final ItemViewSelector<ItemViewModel> itemView = new BaseItemViewSelector<ItemViewModel>() {
        ...
        public void select(ItemView itemView, int position, ItemViewModel itemViewModel) {
            if (position == 0) {
                itemView.set(org.icegeneral.mvvm.BR.viewModel, R.layout.list_news_header);
            } else if (itemViewModel.storiesBean.isHeader()) {
                if (position == 1) {
                    itemViewModel.date.set("今日热闻");
                }
                itemView.set(org.icegeneral.mvvm.BR.viewModel, R.layout.listitem_news_header);
            } else {
                itemView.set(org.icegeneral.mvvm.BR.viewModel, R.layout.listitem_news);
            }
        }
